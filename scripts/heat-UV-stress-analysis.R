...
# 
# y1 = y1 + annotate("text", x = 3, y = 14, label = "***", size = 7, color = "#960101") 
# y2 = y2 + annotate("text", x = 3, y = 13.5, label = "*", size = 7, color = "#960101") 
# 
# 
# short.term.x = plot_grid(y1, y2, y3, y4, y5, align = "h", ncol = 5, rel_widths = c(0.20, 0.20, 0.20,0.20,0.20)) +
...
# 
# png(filename = file.path(getwd(),"/Lifespan-km-plot.png"), width = 8, height = 5, units = "in", res = 600)
# 
# plot(g$plot)
# dev.off()
# print(paste("Saved at ", getwd()))
...
png(filename = file.path(getwd(),"/Heat-km-plot.png"), width = 8, height = 3.5, units = "in", res = 600)
plot(g$plot)
dev.off()
print(paste("Saved at ", getwd()))
print(g$plot)
...
max_y = round(max(df$mean) + 1.0)

control.tox = df[df$Concentration == "Control", ] 
y1 = ggplot(control.tox, aes(Treatment, mean, fill=Concentration)) +
  theme_classic() + labs(x = '', y = 'Mean Lifespan (Hours)', title="Control") +
...
print(res)
write.csv(as.data.frame(res$`E. coli`), file="Heat-stress-stat-results.csv")
write.csv(df, file="Heat-stress-values.csv")

y1 = y1 + annotate("text", x = 3, y = 12.0, label = "***", size = 7, color = "#960101")
y2 = y2 + annotate("text", x = 1, y = 11.4, label = "**", size = 7, color = "#960101") +
  # annotate("text", x = 2, y = 11.0, label = "*", size = 7, color = "#960101") +
  annotate("text", x = 3, y = 11.1, label = "*", size = 7, color = "#960101")
y4 = y4 + annotate("text", x = 3, y = 11.0, label = "*", size = 7, color = "#960101")
...
png(filename = file.path(getwd(),"/UV-km-plot.png"), width = 8, height = 3.5, units = "in", res = 600)
plot(g$plot)
dev.off()
print(paste("Saved at ", getwd()))
print(g$plot)
...
## Prepare dataframe with mean and standard dev
newdat.p = to_survival_from_table_2(data)
## Note that 0.5 was used because every 0.5d is the observation time
newdat.p$lifetime = newdat.p$Time - 0.5
df = newdat.p %>% group_by(Treatment, Concentration) %>% summarise(mean = mean(lifetime), stdv = std.error(lifetime), count = n())
df$upper = df$mean + df$stdv
df$lower = df$mean - df$stdv

max_y = round(max(df$mean) + 0.5)
...
newdat$lifetime = newdat$Time - 0.5
res = DunnettTest(x=newdat$lifetime, g=newdat$group)
print(res)
write.csv(as.data.frame(res$`E. coli`), file="UV-stat-results.csv")
write.csv(df, file="UV-results.csv")

y1 = y1 + annotate("text", x = 3, y = 4.83, label = "***", size = 7, color = "#960101")
y2 = y2 + annotate("text", x = 2, y = 4.30, label = "***", size = 7, color = "#960101") +
  annotate("text", x = 3, y = 3.97, label = "*", size = 7, color = "#960101")
y4 = y4 + annotate("text", x = 2, y = 4.25, label = "***", size = 7, color = "#960101") +  
  annotate("text", x = 3, y = 4.43, label = "***", size = 7, color = "#960101")
